...
class LLMEvalJudge(BaseEvalJudge):
    ...
    async def score(
        ...
        eval_criteria = (
            criteria or trajectory.task.eval_criteria or self.default_criteria
        )
        ...
        try:
            ...
            result = await self.client.create(messages, output_format=JudgeResponse)
            ...
            if result.structured_output:
                ...
                entries = parsed.get("scores") or []
                dimensions = {e["name"]: float(e["score"]) for e in entries}
                reasoning = {e["name"]: e.get("reasoning", "") for e in entries}
                if not dimensions:
                    raise ValueError(
            ...
            # a neutral 5.0 would inflate failing runs.
            dim_scores = [
