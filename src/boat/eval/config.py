...
def _ensure_skill_frontmatter(name: str, content: str) -> str:
    ...
    frontmatter that boat' SkillsTool needs to surface it (name/description),
...
...     model_provider="openai",
    ...
    # Model settings
    model_provider: str = "openai"  # OpenAI-compatible providers only
    ...
    instruction_preset: Optional[str] = None  # truthy → use get_instructions() instead of system_prompt
    tools: List[str] = field(default_factory=lambda: ["coding"])
    max_iterations: int = 30

    # Optimizable text artifacts (used by boat.optim).
    ...
    def _create_tools(self):
        ...
        coding = create_coding_tools(
            workspace=workspace, bash_timeout=self.bash_timeout
        )
    ...
    def to_agent(self, middlewares=None):
        ...
        # Apply tool description overrides (optimizable via boat.optim).
        ...
        if self.instruction_preset:
            ...
            instructions = get_instructions(tool_names=tool_names)
        else:
        ...
        if self.skills:
            ...
            base = (
                Path(self.workspace)
                if self.workspace
                else Path(tempfile.mkdtemp(prefix="boat_skills_"))
            )
            skills_dir = base / ".boat_skills"
