...
against different systems: Boat, direct model calls, orchestrators,
...
from ..types import RunTrajectory, Task, Usage
from .base import Target
from .config import AgentConfig


class AgentEvalTarget(Target):
    """Target that wraps a boat Agent.
...
class BoatTarget(Target):
    ...
    async def run(
        ...
        *,
        ...
        """Execute task with Boat using run_stream to capture full trace.
        ...
        try:
            ...
            context_messages = (
                list(response.context.messages) if response.context else []
            )
...
class CopilotTarget(Target):
    ...
    async def run(
        ...
        try:
            ...
            # Process collected events into boat messages
            for event in all_events:
                ...
                if etype == SessionEventType.ASSISTANT_MESSAGE:
                    ...
                    input_tokens += int(getattr(d, "cache_read_tokens", 0) or 0)
                    input_tokens += int(getattr(d, "cache_write_tokens", 0) or 0)
                    ...
                    if metrics and isinstance(metrics, dict):
                        ...
                        for mm in metrics.values():
                            ...
                            if u:
                                agg_in += int(getattr(u, "input_tokens", 0) or 0)
                                agg_in += int(getattr(u, "cache_read_tokens", 0) or 0)
                                agg_in += int(getattr(u, "cache_write_tokens", 0) or 0)
                                agg_out += int(getattr(u, "output_tokens", 0) or 0)
                            r = getattr(mm, "requests", None)
                            if r:
                                agg_cost += float(getattr(r, "cost", 0) or 0)
                    ...
                    api_dur = getattr(event.data, "total_api_duration_ms", None)
                    if api_dur:
