...
class TaskResult:
    ...
    def total_tokens(self) -> int:
        if self.trajectory.usage:
            return (
                self.trajectory.usage.tokens_input + self.trajectory.usage.tokens_output
            )
        return 0
...
class EvalResults:
    ...
    def get_summaries(self) -> Dict[str, TargetSummary]:
        ...
        for target_name in self.target_names:
            ...
            summaries[target_name] = TargetSummary(
                ...
                avg_iterations_per_task=sum(iterations) / len(iterations)
                if iterations
                else 0,
                total_duration_ms=sum(durations),
                avg_duration_per_task_ms=sum(durations) / len(durations)
                if durations
                else 0,
                total_unique_files=sum(unique_files),
                total_duplicate_reads=sum(duplicate_reads),
                duplicate_read_ratio=sum(duplicate_reads) / total_files
                if total_files > 0
                else 0,
    ...
    def compare_targets(
        ...
        for target_name, summary in summaries.items():
            ...
            if baseline_summary.total_duration_ms > 0:
                dur_diff = (
                    summary.total_duration_ms - baseline_summary.total_duration_ms
                )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "summaries": {
                name: s.to_dict() for name, s in self.get_summaries().items()
            },
    ...
    def save(self, path: Optional[Path] = None) -> Path:
        ...
        Args:
            path: Output path (default: .boat/eval/{run_id}.json)

        Returns:
            Path to saved file
        """
        if path is None:
            output_dir = Path.cwd() / ".boat" / "eval"
...
def list_eval_results(
    ...
    Args:
        output_dir: Directory to search (default: .boat/eval/)

    Returns:
        List of paths, newest first
    """
    output_dir = output_dir or Path.cwd() / ".boat" / "eval"
    if not output_dir.exists():
        return []

    # Support both old benchmark_* and new eval_* patterns
    all_files = list(output_dir.glob("eval_*.json")) + list(
