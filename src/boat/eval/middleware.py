...
class RunMiddleware(BaseMiddleware):
    ...
    async def process_request(self, context: MiddlewareContext):
        ...
        if context.operation == "model_call":
            self.current_iteration = {
                ...
                "output_tokens": 0,
                "tool_calls": [],
                "message_count": len(context.data)
    ...
    async def process_response(self, context: MiddlewareContext, result: Any):
        ...
        if context.operation == "model_call" and self.current_iteration is not None:
            if hasattr(result, "usage"):
                self.current_iteration["input_tokens"] = getattr(
                    result.usage, "tokens_input", 0
                )
                self.current_iteration["output_tokens"] = getattr(
                    result.usage, "tokens_output", 0
                )
            ...
            if tool_name in ("read_file", "Read", "read"):
                path = (
                    ...
                    or parameters.get("filename", "unknown")
                )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        duplicate_reads = (
            total_reads - unique_files if total_reads > unique_files else 0
        )
        ...
        return {
            ...
            "token_growth": [
                (it.get("index", i), it.get("input_tokens", 0))
            ...
            "duplicate_read_ratio": duplicate_reads / total_reads
            if total_reads > 0
            else 0,
            # Compaction metrics
            ...
            "tokens_saved": sum(
                e.get("tokens_saved", 0) for e in self.compaction_events
            ),
            # Errors
            "errors": self.errors,
