...
from .results import EvalResults, TaskResult
from .targets import AgentEvalTarget, BoatTarget

#: Anything that can be passed to ``EvalRunner.run()`` as a target.
#: - ``Target``: used as-is
#: - ``AgentConfig``: wrapped in ``BoatTarget`` (fresh agent per task)
...
class EvalRunner:
    ...
    async def run(
        ...
        Args:
            ...
            persist: If True, save results to ~/.boat/ (DB
    ...
    def _failed_task_result(
        ...
        trajectory = RunTrajectory(task=task, messages=[], success=False, error=message)
        return TaskResult(
    ...
    async def _run_single_task(
        ...
        When a BoatTarget has no explicit workspace, an isolated temp
        ...
        needs_temp = isinstance(target, BoatTarget) and target.config.workspace is None
        task_workspace = None
        if needs_temp:
            task_workspace = Path(
                tempfile.mkdtemp(prefix=f"eval_{target.name}_{task_id}_")
            )

        try:
            if isinstance(target, BoatTarget):
                if needs_temp:
