...
from ..component_config import Component, ComponentModel
...
class AgentConfig(BaseModel):
    ...
    output_format_schema: Optional[Dict[str, Any]] = (
...
class Agent(Component[AgentConfig], BaseAgent):
    ...
    component_provider_override = "boat.agents.Agent"

    def _should_create_span(self) -> bool:
        """Check if we should create OpenTelemetry spans."""
        return os.getenv("BOAT_ENABLE_OTEL", "false").lower() in (
            "true",
            "1",
            "yes",
        )
    ...
    async def run(
        ...
        Args:
            ...
            cancellation_token: Optional token for cancelling
                execution
            persist: If True, save the run to ~/.boat/ (DB
        ...
        if self._should_create_span():
            try:
                ...
                response = await self._run_internal(task, context, cancellation_token)
        else:
            response = await self._run_internal(task, context, cancellation_token)
        ...
        if persist:
            try:
                ...
                await store.save_agent_run(self, response, trace_id=trace_id)
            except Exception as e:
                import logging

                logging.getLogger(__name__).warning(f"Failed to persist run: {e}")
    ...
    async def _run_internal(
        ...
        try:
            async for item in self.run_stream(
                None, working_context, cancellation_token, False, False
            ):
    ...
    async def run_stream(
        ...
        try:
            ...
            # Pass empty list since context messages are added inside _prepare_llm_messages
            llm_messages = await self._prepare_llm_messages([], context=working_context)
            ...
            if self.start_hooks or self.end_hooks:
                ...
                for hook in self.start_hooks:
                    ...
                    if injection:
                        hook_msg = UserMessage(content=injection, source="hook")
                ...
                try:
                    ...
                    if effective_stream_tokens:
                        ...
                        completion_result = ChatCompletionResult(
                            ...
                            usage=Usage(
                                duration_ms=streaming_usage.duration_ms
                                if streaming_usage
                                else 0,
                                llm_calls=1,
                                tokens_input=streaming_usage.tokens_input
                                if streaming_usage
                                else 0,
                                tokens_output=streaming_usage.tokens_output
                                if streaming_usage
                                else 0,
    ...
    async def _execute_tool_calls_parallel(
        ...
        following Anthropic's best practice of parallel tool execution.
            ...
            if tool.approval_mode == ApprovalMode.ALWAYS:
                ...
                if existing_approval is None:
                    ...
                    approval_request = working_context.add_approval_request(
                        tool_call=tool_call, tool_name=tool_call.tool_name
                    )
                    ...
                    approval_event = ToolApprovalEvent(
                        source=self.name, approval_request=approval_request
                    )
            ...
            if tool.supports_streaming():
                ...
                async for item in self.middleware_chain.execute_stream(
                    ...
                    if isinstance(item, ToolResult):
                        ...
                        # Check for approval pause
                        if isinstance(item, ToolApprovalEvent):  # type: ignore
                            return  # PAUSE - middleware requested approval
