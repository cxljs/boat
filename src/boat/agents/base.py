...
class BaseAgent(ComponentBase[BaseModel], ABC):
    ...
    All agents in the framework must inherit from this base class
    ...
    def __init__(
        ...
        description: str = "",
        tools: Optional[List[Union[BaseTool, Callable]]] = None,  # type: ignore
        ...
        start_hooks: Optional[List] = None,  # type: ignore
        end_hooks: Optional[List] = None,  # type: ignore
        **kwargs: Any,
    ):
        """
        Args:
        ...
        self.tools: List[BaseTool] = self._process_tools(tools or [])  # type: ignore
        self.memory = memory
        ...
        self.start_hooks = start_hooks or []  # type: ignore
        self.end_hooks = end_hooks or []  # type: ignore
    ...
    def _process_tools(self, tools: List[Union[BaseTool, Callable]]) -> List[BaseTool]:  # type: ignore
        """
        ...
        for tool in tools:  # type: ignore
            if isinstance(tool, BaseTool):
                processed.append(tool)  # type: ignore
            elif callable(tool):  # type: ignore
                processed.append(FunctionTool(tool))  # type: ignore
            else:
    ...
    async def run(
        ...
        Implementations operate on an isolated working context and do not
        mutate self.context; the resulting state is returned in
        response.context. For multi-turn conversations, pass the previous
        response's context to the next call::
        ...
        Args:
            task: Optional new task (can continue from context alone
                if not provided)
            context: Existing context to continue from. If not provided,
                the agent's own context is deep-copied as starting point
            cancellation_token: Optional token for cancelling execution
            persist: If True, save the run to ~/.boat/ (DB index
    ...
    def run_stream(
        ...
        Yields:
            Messages, events (if verbose=True), ChatCompletionChunks (if stream_tokens=True),
            and final AgentResponse with usage stats
    ...
    async def _prepare_llm_messages(
            ...
            try:
                ...
                for memory_content in memory_result.results:
                    if isinstance(memory_content.content, str):
                        context_items.append(memory_content.content)  # type: ignore
                    else:
                        import json

                        context_items.append(json.dumps(memory_content.content))  # type: ignore
        ...
        for tool in self.tools:
            if hasattr(tool, "get_system_prompt_section"):
                try:
                    section = tool.get_system_prompt_section()  # type: ignore
                    if section:
                        system_content += section  # type: ignore
                except Exception:
                    pass

        messages.append(SystemMessage(content=system_content, source="system"))  # type: ignore
