...
class OpenAIChatCompletionClient(
    ...
    component_provider_override = "boat.llm.OpenAIChatCompletionClient"
    ...
    async def create_stream(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            tools: Optional function definitions for tool calling
            output_format: Optional Pydantic model for structured output
                           (Note: streaming structured output not fully supported yet)
            stream_options: Stream options (defaults to {"include_usage": True} to enable token tracking)
            **kwargs: Additional OpenAI parameters

        Yields:
            ChatCompletionChunk objects with incremental response data (final chunk includes usage
    ...
    def _estimate_cost(self, usage: CompletionUsage) -> float:
        ...
        # Approximate pricing
        pricing = {
            "gpt-5.6-sol": {"input": 0.03 / 1000, "output": 0.06 / 1000},
            "gpt-5.5": {"input": 0.01 / 1000, "output": 0.03 / 1000},
        }

        model_pricing = pricing.get(self.model, pricing["gpt-5.6-sol"])
