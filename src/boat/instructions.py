...
Informed by Claude Code system prompt patterns.
"""
...
TOOL_GUIDES = {
    ...
    "task": "- **task**: Delegate complex sub-tasks to a specialist agent with isolated context. Use `prompt` and `agent_type` (explore, general) parameters. The sub-agent runs independently and returns only its final result.",
...
_PERSONA = """
You are a capable general-purpose agent. You solve tasks by combining reasoning, tool use,
