"""Context engineering tools for Boat.
...
class TaskTool(BaseTool):
    ...
    Example:
        from boat import Agent
        from boat.tools import TaskTool
    ...
    def __init__(
        ...
        super().__init__(
            ...
            description=(
                ...
                "(3) the task has clear input/output boundaries. "
                "Agent types: 'explore' (codebase search), 'general' (all tools)."
            ),
        )
    ...
    def _get_tools_for_type(self, agent_type: str, config: dict) -> List[BaseTool]:  # type: ignore
        ...
        if tool_names is None:
            ...
            if self.coordinator:
                return [t for t in self.coordinator.tools if t.name != "task"]
            return []

        # Specific tool set - create minimal tools
        tools = []
        from .core_tools import ThinkTool
...
def _get_workspace() -> Path:
    """Get the boat workspace directory."""
    workspace = Path.cwd() / ".boat"
...
class TodoWriteTool(BaseTool):
    ...
    async def execute(self, parameters: Dict[str, Any]) -> ToolResult:
        ...
        current_msg = (
            f"Current: {current['activeForm']}" if current else "No task in progress"
        )
...
class TodoListSessionsTool(BaseTool):
    ...
    async def execute(self, parameters: Dict[str, Any]) -> ToolResult:
        ...
        for session in sessions:
            created = datetime.fromtimestamp(session["created"]).strftime(
                "%Y-%m-%d %H:%M"
            )
...
class SkillsTool(BaseTool):
    ...
    def __init__(
        ...
        Args:
            ...
            user_path: Path to user skills (defaults to ~/.boat/skills/)
        ...
        if user_path:
            ...
            # Default user path
            default_user = Path.home() / ".boat" / "skills"
    ...
    def _discover_skills(self) -> Dict[str, tuple]:  # type: ignore
        """Discover all skills from configured paths."""
        skills: Dict[str, tuple] = {}  # type: ignore
    ...
    async def execute(self, parameters: Dict[str, Any]) -> ToolResult:
        ...
        if action == "list":
            if not discovered:
                paths_str = (
                    "\n".join(f"  - {p}" for p in self.skill_paths)
                    if self.skill_paths
                    else "  (no paths configured)"
                )
                result = (
                    "No skills found.\n\n"
...
class MultiEditTool(BaseTool):
    ...
    async def execute(self, parameters: Dict[str, Any]) -> ToolResult:
        ...
        if not edits:
            return ToolResult(
                success=False, result="", error="'edits' list is required"
            )
        ...
        try:
            ...
            for i, edit in enumerate(edits):
                ...
                applied.append(
                    f"Edit {i + 1}: replaced {len(old_str)} chars with {len(new_str)} chars"
                )
            ...
            result = (
                f"Successfully applied {len(edits)} edit(s) to {path_str}:\n"
                + "\n".join(applied)
            )
