"""
MCPTool - Bridge between MCP tools and Boat tools.

This module provides the MCPTool class that wraps MCP server tools
as Boat BaseTool instances, enabling seamless integration.
"""
...
class MCPTool(BaseTool):
    """
    Wraps an MCP server tool as a Boat tool.

    This bridge allows MCP tools to be used transparently
    in Boat agents alongside native tools.
    The tool automatically handles:
    - Parameter validation using MCP schemas
    - Execution via MCP client
    - Result conversion between MCP and Boat formats
    ...
    async def execute(self, parameters: Dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            # Convert MCP result to Boat ToolResult
