"""Run the real GEPA library against boat, with faithful cost tracking.
...
- agent + judge: accumulated inside ``BoatGepaAdapter.evaluate`` (we run the
  boat agent and our judge there, exactly like ``BaseOptimizer._eval``).
...
gepa is synchronous, so we run it in a worker thread and drive boat' async
...
from ..eval import AgentConfig, BoatTarget, Dataset, EvalRunner
from ..llm import BaseChatCompletionClient
from ..messages import UserMessage
from ..types import Task, Usage
from .base import CostLog
from .spec import Edit, InstructionTunable, Operation, OptimizationSpec
from .trace import OptimizationTrace, usage_dict
...
def _require_gepa() -> None:
    if not GEPA_AVAILABLE:
        raise ImportError(
            ...
            "The rest of boat.optim works without it."
        )


class _CostTrackingReflectionLM:
    """Wraps a boat client as a GEPA reflection LM, counting tokens.
    ...
    def __call__(self, prompt: Any) -> str:
        if isinstance(prompt, str):
            ...
            messages = [
                UserMessage(content=m.get("content", ""), source="gepa") for m in prompt
            ]
...
class BoatGepaAdapter:
    """GEPA adapter that executes boat ``AgentConfig`` candidates.
    ...
    each task with the boat agent + judge, and return per-example scores and
    ...
    def config_from_candidate(self, candidate: Dict[str, str]) -> AgentConfig:
        edits = [
            Edit(
                Operation.REWRITE,
                self._kind_by_name.get(name, "instructions"),
                name,
                value=text,
            )
        ...
        return replace(
            cfg, name=f"gepa-{abs(hash(tuple(sorted(candidate.items())))) % 100000}"
        )
    ...
    def evaluate(
        ...
        scores = asyncio.run(self.runner.evaluate(BoatTarget(cfg), list(batch)))
        ...
        for task, s in zip(batch, scores):
            ...
            feedback = (
                "; ".join(f"{k}: {v}" for k, v in s.reasoning.items())
                or f"score {s.overall}"
            )
            if trajectories is not None:
                trajectories.append(
                    {
                        "input": task.input,
                        "output": s.get_final_response(),
                        "feedback": feedback,
                    }
                )
            per_task.append(
        ...
        return EvaluationBatch(
            outputs=outputs, scores=score_vals, trajectories=trajectories
        )
...
@dataclass
class GepaRunResult:
    """Result of a real-GEPA run over boat."""
    ...
    history: List[
        Dict[str, Any]
    ]  # per-evaluate records (candidate + per-task responses/scores)
...
class _GepaTraceCallback:
    ...
    def on_proposal_end(self, event: Dict[str, Any]) -> None:
        self._rec(
            "proposal_end", event
        )  # carries prompts + raw_lm_outputs + new_instructions
...
async def optimize_with_gepa(
    ...
    **gepa_kwargs: Any,
    ...
    """Optimize a boat agent with the real GEPA library.
    ...
    adapter = BoatGepaAdapter(base_config, spec, runner, cost, run_trace)
