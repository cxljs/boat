...
Evaluation reuses the boat ``eval`` module unchanged: every proposed
...
class CostLog:
    ...
    def to_dict(self) -> Dict[str, Dict[str, Optional[float]]]:
        ...
        def source_usd(u: Usage) -> Optional[float]:
            ...
            return (
                0.0 if u.llm_calls == 0 else None
            )  # no work -> $0; worked but unknown -> None

        per_source_usd = [
            source_usd(self.agent),
            source_usd(self.judge),
            source_usd(self.reflection),
        ]
        out["total"]["usd"] = (
    ...
    def summary(self) -> str:
        ...
        for src in ("agent", "judge", "reflection", "total"):
            ...
            lines.append(
                f"{src:12s} {r['tokens_total']:12,d} {r['llm_calls']:10d} {usd:>10s}"
            )
        return "\n".join(lines)
...
class Candidate:
    ...
    def avg(self) -> float:
        return (
            sum(s.overall for s in self.scores) / len(self.scores)
            if self.scores
            else 0.0
        )
...
class BaseOptimizer(ABC):
    ...
    async def optimize(self, seed: AgentConfig) -> OptimizationResult:
        ...
        seed_cand = Candidate(
            seed,
            await self._eval_full(seed, purpose="seed"),
            parent=None,
            rationale="seed",
        )
        ...
        self.trace.record(
            ...
            rationale="seed",
            avg=seed_cand.avg,
            task_scores=seed_cand.task_scores,
        ...
        for r in range(self.rounds):
            ...
            self.trace.record(
                "parents",
                self._round,
                parents=[c.config.name for c in parents],
    ...
    async def _eval(self, config: AgentConfig, tasks: List[Task]) -> List[EvalScore]:
        scores = await self.runner.evaluate(BoatTarget(config), tasks)
    ...
    async def _eval_full(
        self, config: AgentConfig, purpose: str = "full"
    ) -> List[EvalScore]:
    ...
    async def _passes_gate(self, parent: Candidate, cfg: AgentConfig) -> bool:
        ...
        self.trace.record(
            ...
            parent_scores={
                (t.id or t.name): parent_lookup.get(t.id or t.name, 0.0) for t in mb
            },
