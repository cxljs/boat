...
from .base import BaseOptimizer, Candidate, task_key
from .spec import Component, Edit, Operation, ProposalMode
...
class _EditOut(BaseModel):
    ...
    name: str = Field(
        description="Component name to edit, e.g. instructions or skill:debug"
    )
    value: str = Field(
        default="", description="New text (rewrite/add) or catalog name (tool)"
    )
    rationale: str = Field(
        default="", description="Why this edit fixes observed failures"
    )
...
class ReflectiveOptimizer(BaseOptimizer):
    ...
    def __init__(
        self, *args, reflector: BaseChatCompletionClient, weak_k: int = 3, **kwargs
    ):
    ...
    async def propose(
        ...
        if reflection is not None and reflection.edits:
            edits = [
                e for e in (self._to_edit(e) for e in reflection.edits) if e is not None
            ]
        if edits:
    ...
    def _build_prompt(self, components: List[Component], weak: List[EvalScore]) -> str:
        ...
        for c in components:
            ...
            lines.append(
                f"## {c.name}  (kind={c.kind}, mode={c.mode.value}, ops=[{ops}])"
            )
