...
from .base import BaseOptimizer, Candidate
...
class MIPROOptimizer(BaseOptimizer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self._last_proposal_call: Tuple[
            List[Dict[str, str]], Optional[str], Optional[Usage], Optional[str]
        ] = ([], None, None, None)
    ...
    async def _summarize_tasks(self) -> str:
        ...
        lines = [
            "Tasks the agent must handle (with example expected outputs where available):\n"
        ]
        for t in sample:
