"""
Entity registry for Boat WebUI.
...
class EntityRegistry:
    """Registry for managing discovered Boat entities."""
    ...
    def __init__(self, entities_dir: Optional[str] = None) -> None:
        ...
        self.scanner = BoatScanner(entities_dir) if entities_dir else None
    ...
    def register_from_file(self, file_path: str, entity_id: str) -> Optional[Entity]:
        ...
        try:
            ...
            entity_obj = None
            for var_name in ["agent", "orchestrator"]:
                if hasattr(module, var_name):
                    entity_obj = getattr(module, var_name)
                    break

            if not entity_obj:
                logger.error(f"No agent or orchestrator found in {file_path}")
                return None
    ...
    def _create_entity_info_from_object(
        ...
        """
        from .models import AgentInfo, OrchestratorInfo
        ...
        # Try to determine type by checking for Boat base classes
        try:
