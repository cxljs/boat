...
Five small MCP servers that ship inside the package, so `pip install boat`
...
```bash
python -m boat.webui.mcp.servers.basic_server           # stdio
python -m boat.webui.mcp.servers.basic_server --http    # streamable HTTP
```
...
```bash
python -m boat.webui.mcp.servers.auth_server     # http://127.0.0.1:8931/mcp
```

Connect without the `Authorization` header first to watch the 401 challenge on
the wire, then add `{"Authorization": "Bearer boat-lab-token"}`.

Requires `mcp>=2.0.0` (`pip install "boat[mcp]"`).
