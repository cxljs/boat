...
if TYPE_CHECKING:
    from ..store.store import BoatStore
...
class EvalJobManager:
    ...
    def __init__(self, store: BoatStore) -> None:
        self._store = store
    ...
    async def _execute(
        ...
        try:
            ...
            from ..eval.targets import BoatTarget
            ...
            for tid in target_ids:
                ...
                if tc_data.get("target_type") == "boat":
                    config_dict = tc_data.get("config", {})
                    if config_dict:
                        agent_config = AgentConfig.from_dict(config_dict)
                        targets.append(BoatTarget(agent_config))
                    else:
                        logger.warning(f"Target {tid} has no config, skipping")
            ...
            for target in targets:
                ...
                for task in dataset.tasks:
                    ...
                    await self._store.save_eval_result(eval_run_id, task_result)
