...
def _get_store(request: Request):
    ...
    if store is None:
        raise HTTPException(
            status_code=503,
            detail="Persistence not available — install boat[persist]",
        )
    return store
...
async def list_builtins(request: Request):
    ...
    from ..eval.dataset import list_builtin_datasets
...
async def add_task(
    ...
    store = _get_store(request)
    result = await store.add_task(dataset_id, body.model_dump(exclude_none=True))
    if result is None:
        raise HTTPException(status_code=404, detail="Dataset not found")
    return result
