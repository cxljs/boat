...
from .discovery import BoatScanner
...
from .server import BoatWebUIServer, create_app
...
def launch(
    ...
    """Launch Boat WebUI with entities discovery.

    This is the main entry point for launching the web interface. It will scan
    the specified directory for Boat entities and start a web server.
...
class WebUIServer:
    ...
    def register_entity(self, entity_id: str, entity_obj: Any) -> None:
        ...
        Args:
            ...
            entity_obj: Entity object (Agent or Orchestrator)
...
def serve(
    ...
    Simple API for serving agents and orchestrators.
    Entities can be provided directly as objects or discovered from a directory.

    Args:
        entities: List of entity objects (Agent, Orchestrator) to serve
    ...
    Example:
        ```python
        from boat import Agent
        from boat.orchestration import RoundRobinOrchestrator
        from boat.webui import serve
    ...
    server = WebUIServer(
        entities_dir=entities_dir if entities_dir else None, port=port, host=host
    )
...
def scan_entities(directory: str) -> List[Entity]:
    """Scan a directory for Boat entities.
    ...
    Example:
        ```python
        from boat.webui import scan_entities
