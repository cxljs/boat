"""
Execution engine for Boat WebUI.

Streams raw Boat events with session context management.
"""
...
class ExecutionEngine:
    """Engine for executing Boat entities with session management."""
    ...
    async def execute_agent_stream(
        ...
        """Stream raw Boat events with session management.
        ...
        Yields:
            Server-sent events containing raw Boat events
        """
        ...
        try:
            ...
            async for event in agent.run_stream(
                ...
                if isinstance(event, AgentResponse):
                    ...
                    event_data = {
                        "messages": [msg.model_dump() for msg in event.context.messages]
                        if event.context
                        else [],
    ...
    async def execute_orchestrator_stream(
        ...
        # Use both id and name - prefer id for consistency
        entity_id = getattr(orchestrator, "id", None) or getattr(
        ...
        try:
            ...
            async for event in orchestrator.run_stream(
                ...
                wrapped_event = WebUIStreamEvent(session_id=session_id, event=event)
                yield f"data: {wrapped_event.model_dump_json()}\n\n"
