        ...
        if schema is not None:
            try:
                self.requested_schema = (
                    ...
                    else schema.model_dump(by_alias=True)
                )
            except Exception:
...
class McpPlayground:
    ...
    def __init__(self) -> None:
        from boat.tools import MCPClientManager
    ...
    async def _handle_elicitation(self, server_id: str, params: Any) -> Any:
        ...
        try:
            reply = await asyncio.wait_for(
                pending.future, timeout=INPUT_TIMEOUT_SECONDS
            )
            return ElicitResult(
    ...
    def resolve_input(
        self, input_id: str, action: str, content: Optional[Dict[str, Any]]
    ) -> bool:
...
def get_spec_support() -> Dict[str, Any]:
    ...
    check(
        ...
        lambda: (
            hasattr(t, "DiscoverRequest") and t.LATEST_PROTOCOL_VERSION >= "2026-07-28"
        ),
    )
    check(
        "mrtr",
