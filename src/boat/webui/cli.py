...
def webui(
    ...
    """Launch Boat WebUI server.

    Args:
        entities_dir: Directory to scan for Boat entities.
    ...
    if entities_dir is not None and app is None:
        ...
        print(f"🔍 Scanning {entities_path} for agents and orchestrators...")

        # Quick discovery check to provide feedback
        scanner = BoatScanner(str(entities_path))
        try:
            ...
            if discovered:
                ...
                print(f"⚠️  No agents or orchestrators found in {entities_path}")
    ...
    print(f"🚀 Starting Boat WebUI on http://{host}:{port}")
...
def main() -> None:
    """CLI entry point for boatui command."""
    parser = argparse.ArgumentParser(
        description="Launch Boat WebUI",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  boatui                    # Scan current directory
  boatui --dir ./agents     # Scan specific directory
  boatui --port 8000        # Use different port
  boatui --no-open          # Don't open browser
  boatui --reload           # Enable auto-reload for development
        """,
    )
