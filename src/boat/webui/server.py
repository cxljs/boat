...
from .models import AddExampleRequest, Entity, HealthResponse
...
def _boat_version() -> str:
    from boat import __version__

    return __version__


class RunEntityRequest(BaseModel):
    """Request model for entity execution - minimal wrapper around Boat types."""
    ...
    approval_responses: Optional[List[ToolApprovalResponse]] = Field(
        default=None,
        description="Tool approval responses to inject into session context",
    )


class BoatWebUIServer:
    """FastAPI server for Boat WebUI."""
    ...
    def __init__(
        ...
        # Persistence store (optional — requires boat[persist])
        self._store = None
        try:
            from ..store import BoatStore

            self._store = BoatStore()
        except ImportError:
            logger.info(
                "sqlmodel not installed — persistence disabled. "
                "Install with: pip install boat[persist]"
            )
    ...
    def create_app(self) -> FastAPI:
        ...
        async def lifespan(app: FastAPI):
            # Startup
            logger.info("Starting Boat WebUI Server")
            if store is not None:
                await store.initialize()
                app.state.store = store
                from .eval_jobs import EvalJobManager

                app.state.eval_jobs = EvalJobManager(store)
                logger.info("Persistence store initialized")
            try:
                from boat.tools import MCP_AVAILABLE

                if MCP_AVAILABLE:
                    from .mcp_playground import McpPlayground
            ...
            if store is not None:
                ...
                if close is not None:
                    await close()
            logger.info("Shutting down Boat WebUI Server")

        app = FastAPI(
            title="Boat WebUI",
            description="Web interface for interacting with Boat entities",
            version=_boat_version(),
            lifespan=lifespan,
        )
        ...
        # MCP playground (requires boat[mcp] with mcp>=2.0)
        try:
            from boat.tools import MCP_AVAILABLE
    ...
    def _register_routes(self, app: FastAPI) -> None:
            ...
            async def cancellable_generator():
                ...
                try:
                    if entity_info.type == "agent":
                        ...
                        event_generator = (
                            self.execution_engine.execute_orchestrator_stream(
        ...
        async def add_example(request: AddExampleRequest):
            ...
            try:
                ...
                # Note: Examples are at the root of the repository, not in boat subdirectory
                ...
                temp_dir = Path(tempfile.gettempdir()) / "boat_examples"
...
def create_app(
    ...
    **kwargs: Any,
    ...
    """Create FastAPI app for Boat WebUI.
    ...
    """
    server = BoatWebUIServer(entities_dir=entities_dir, **kwargs)
