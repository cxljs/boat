...
def get_playground(request: Request) -> Any:
    ...
    if playground is None:
        raise HTTPException(
            status_code=503,
            detail="MCP playground not initialized (mcp not installed?)",
        )
    return playground
...
async def spec_support() -> Dict[str, Any]:
    ...
    from .mcp_playground import get_spec_support
...
async def presets() -> List[Dict[str, Any]]:
    from .mcp_playground import get_presets
...
async def add_server(request: Request, body: AddServerRequest) -> Dict[str, Any]:
    from boat.tools import HTTPServerConfig, StdioServerConfig
    ...
    if body.transport == "stdio":
        if not body.command:
            raise HTTPException(
                status_code=400, detail="stdio transport requires 'command'"
            )
        ...
        if not body.url:
            raise HTTPException(
                status_code=400, detail=f"{body.transport} transport requires 'url'"
            )
        ...
        raise HTTPException(
            status_code=400, detail=f"Unknown transport: {body.transport}"
        )
...
def _require_connected(playground: Any, server_id: str) -> Any:
    ...
    if not manager.is_connected(server_id):
        raise HTTPException(
            status_code=409, detail=f"Server '{server_id}' is not connected"
        )
    return manager
...
async def reply_input(
    request: Request, input_id: str, body: InputReplyRequest
) -> Dict[str, Any]:
    playground = get_playground(request)
