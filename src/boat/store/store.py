"""Async data store for Boat persistence.

BoatStore uses SQLModel + SQLAlchemy async engine for the DB index,
...
class BoatStore:
    ...
    async def initialize(self) -> None:
        ...
        logger.info(f"BoatStore initialized: {self._connection_string}")
    ...
    async def save_agent_run(
        ...
        file_path.write_text(json.dumps(json_data, indent=2, default=str))
    ...
    async def import_builtin_dataset(self, name: str) -> Dict[str, Any]:
        ...
        from ..eval.dataset import load_builtin_dataset
    ...
    async def list_target_configs(self) -> List[Dict[str, Any]]:
        ...
        async with self.session() as sess:
            result = await sess.exec(
                select(DBTargetConfig).order_by(col(DBTargetConfig.created_at).desc())
            )
            return [_model_to_dict(c) for c in result.all()]

    async def get_target_config(self, config_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def get_eval_run(self, eval_run_id: str) -> Optional[Dict[str, Any]]:
    ...
    async def save_eval_run_from_results(
        ...
        logger.info(f"Saved eval run {eval_run_id} with {result_count} results")
        return eval_run_id
    ...
    async def save_eval_result(
        ...
        db_result = task_result_to_db_eval_result(eval_run_id, task_result, run_id)
    ...
    async def get_eval_results(self, eval_run_id: str) -> List[Dict[str, Any]]:
        """Get all results for an eval run."""
        async with self.session() as sess:
            result = await sess.exec(
                select(DBEvalResult).where(DBEvalResult.eval_run_id == eval_run_id)
            )
            return [_model_to_dict(r) for r in result.all()]

    async def get_eval_result(self, result_id: str) -> Optional[Dict[str, Any]]:
...
_default_store: Optional[BoatStore] = None


def get_default_store() -> BoatStore:
    """Get or create the default BoatStore singleton.

    Uses SQLite at ~/.boat/boat.db.
    Initialization is lazy — the DB is created on first async call.
    """
    global _default_store
    if _default_store is None:
        _default_store = BoatStore()
