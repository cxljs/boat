...
class DBDataset(SQLModel, table=True):
    ...
    categories: Optional[List[str]] = Field(default=None, sa_column=Column(SA_JSON))
...
class DBTask(SQLModel, table=True):
    ...
    eval_criteria: Optional[List[str]] = Field(default=None, sa_column=Column(SA_JSON))
    rubric: Optional[Dict[str, str]] = Field(default=None, sa_column=Column(SA_JSON))
    metadata_json: Optional[Dict[str, Any]] = Field(
        default=None, sa_column=Column("metadata", SA_JSON)
    )
    created_at: datetime = Field(default_factory=datetime.utcnow)
...
class DBTargetConfig(SQLModel, table=True):
    ...
    id: str = Field(default_factory=_short_uuid, primary_key=True)
    name: str = Field(index=True)
    target_type: str = "boat"  # "boat" | "claude_code" | "discovered_agent"
    config: Optional[Dict[str, Any]] = Field(default=None, sa_column=Column(SA_JSON))
...
class DBEvalRun(SQLModel, table=True):
    ...
    dataset_name: str = ""
    status: str = (
        "pending"  # "pending" | "running" | "completed" | "error" | "cancelled"
    )

    target_ids: Optional[List[str]] = Field(default=None, sa_column=Column(SA_JSON))
    target_names: Optional[List[str]] = Field(default=None, sa_column=Column(SA_JSON))
...
class DBEvalResult(SQLModel, table=True):
    ...
    reasoning: Optional[Dict[str, str]] = Field(default=None, sa_column=Column(SA_JSON))
