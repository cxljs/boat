...
Example:
    ...
    from boat.compaction import HeadTailCompaction
...
class HeadTailCompaction:
    ...
    def _count_tokens(self, messages: List[Message]) -> int:
        ...
        if self._encoder is None:
            ...
            return sum(
                len(str(getattr(m, "content", "") or "")) // 4 + 10 for m in messages
            )
            ...
            if isinstance(msg, AssistantMessage) and msg.tool_calls:
                ...
                while j < len(messages) and call_ids:
                    if isinstance(messages[j], ToolMessage):
                        tool_msg = messages[j]
                        if tool_msg.tool_call_id in call_ids:  # type: ignore
                            group_indices.append(j)
                            call_ids.remove(tool_msg.tool_call_id)  # type: ignore
                    j += 1
