"""
Deterministic loop hooks for Boat.
...
Example:
    ...
    from boat.hooks import PlanningHook, CompletionCheckHook
    from boat.tools import create_coding_tools, create_todo_tools
...
class CompositeTermination(TerminationCondition):
    ...
    def __init__(self, conditions: List[TerminationCondition], mode: str = "any"):
...
class CompletionCheckHook(BaseEndHook):
    ...
    async def on_end(self, context: LoopContext) -> Optional[str]:
        ...
        from .tools.context_tools import load_todos
...
class LLMCompletionCheckHook(BaseEndHook):
    ...
    def _build_conversation_summary(
        ...
        for msg in messages:
            ...
            if total_chars + len(line) > max_chars:
                lines.append(
                    f"... ({len(messages) - len(lines)} more messages truncated)"
                )
                break
    ...
    async def on_end(self, context: LoopContext) -> Optional[str]:
        ...
        # Ask the judge model
        from .messages import SystemMessage as SM, UserMessage as UM
    ...
    def __repr__(self) -> str:
        return f"LLMCompletionCheckHook(termination={self.termination!r})"
