...
from boat.tools.context_tools import (
    MultiEditTool,
    SkillsTool,
    TaskTool,
...
class TestTodoWriteTool:
    ...
    async def test_missing_required_fields(self, temp_todo_path):
        ...
        result = await tool.execute(
            {"todos": [{"status": "pending", "activeForm": "Test"}]}
        )
        assert result.success is False
        ...
        # Missing status
        result = await tool.execute(
            {"todos": [{"content": "Test", "activeForm": "Test"}]}
        )
        assert result.success is False
...
class TestTodoSessions:
    ...
    def session_dir(self, temp_dir):
        ...
        import boat.tools.context_tools as ctx
        ...
        result = await tool.execute(
            {
                ...
                "edits": [{"old_string": "Hello, World!", "new_string": "Hi there!"}],
            }
        )
...
class TestTaskTool:
    ...
    def test_parameters_schema(self):
        ...
        assert params["properties"]["agent_type"]["enum"] == [
            "explore",
            "general",
        ]
    ...
    async def test_execute_no_model_client(self):
        ...
        result = await tool.execute({"prompt": "Test task", "description": "Test"})
        ...
        result = await tool.execute(
            {
                ...
                "edits": [
                    ...
                    {
                        "old_string": "computed_value = calculate",
                        "new_string": "output = calculate",
                    },
                    {
                        "old_string": "return computed_value * 2",
                        "new_string": "return output * 2",
                    },
                ],
            }
        )
