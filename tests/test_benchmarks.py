...
from boat.eval.base import EvalJudge
from boat.messages import AssistantMessage, UserMessage
...
class TestEvalResults:
    ...
    def test_get_summaries(self):
        ...
        for task_id in ["task_1", "task_2", "task_3"]:
            results.add_result(
                self._create_mock_result(task_id, "baseline", tokens=1000)
            )
            results.add_result(
                self._create_mock_result(task_id, "optimized", tokens=800)
            )
    ...
    def test_compare_targets(self):
        ...
        for task_id in ["task_1", "task_2"]:
            results.add_result(
                self._create_mock_result(task_id, "baseline", tokens=1000, score=7.0)
            )
            results.add_result(
                self._create_mock_result(task_id, "optimized", tokens=600, score=8.0)
            )
        ...
        async def my_runner(task: Task) -> RunTrajectory:
            return RunTrajectory(
                ...
                messages=[
                    UserMessage(content=task.input, source="user"),
                    AssistantMessage(
                        content=f"Completed: {task.name}", source="assistant"
                    ),
                ],
                success=True,
                usage=Usage(
    ...
    async def test_runner_with_task_filter(self):
        ...
        async def simple_runner(task: Task) -> RunTrajectory:
            return RunTrajectory(
                ...
                success=True,
                usage=Usage(
                    duration_ms=50, llm_calls=1, tokens_input=50, tokens_output=50
                ),
            )
        ...
        async def baseline_runner(task: Task) -> RunTrajectory:
            return RunTrajectory(
                ...
                messages=[
                    UserMessage(content=task.input, source="user"),
                    AssistantMessage(
                        content=f"Baseline: {task.name}", source="assistant"
                    ),
                ],
                success=True,
                usage=Usage(
                    duration_ms=500, llm_calls=10, tokens_input=500, tokens_output=500
                ),
            )
        ...
        async def optimized_runner(task: Task) -> RunTrajectory:
            return RunTrajectory(
                ...
                messages=[
                    UserMessage(content=task.input, source="user"),
                    AssistantMessage(
                        content=f"Optimized: {task.name}", source="assistant"
                    ),
                ],
                success=True,
                usage=Usage(
                    duration_ms=300, llm_calls=6, tokens_input=300, tokens_output=300
                ),
            )
