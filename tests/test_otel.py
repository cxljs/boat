...
from boat.middleware import MiddlewareContext
from boat.otel import OTelMiddleware, _is_enabled, auto_instrument
...
class TestOTelConfig:
    ...
    def test_enabled_with_env_var(self):
        ...
        for value in test_cases:
            with patch.dict(os.environ, {"BOAT_ENABLE_OTEL": value}):
...
class TestOTelMiddleware:
    ...
    def test_middleware_disabled_when_otel_off(self):
        ...
        with patch.dict(os.environ, {"BOAT_ENABLE_OTEL": "false"}):
    ...
    async def test_middleware_handles_missing_otel_libs(self):
        ...
        with patch.dict(os.environ, {"BOAT_ENABLE_OTEL": "true"}):
            with patch("boat.otel.OTEL_AVAILABLE", False):
                middleware = OTelMiddleware()
                assert middleware._enabled is False
...
class TestIntegration:
    ...
    @pytest.mark.skipif(
        not HAS_OPENTELEMETRY, reason="opentelemetry package not installed"
    )
    @pytest.mark.asyncio
    async def test_end_to_end_with_mock_tracer(self):
        ...
        with patch.dict(
            ...
            with patch("boat.otel._setup_telemetry") as mock_setup:
                ...
                with patch("opentelemetry.context.attach") as mock_attach:
                    with patch("opentelemetry.context.detach") as mock_detach:
                        with patch(
                            ...
                            async for item in middleware.process_response(
                                result_context, mock_result
                            ):
                                pass
