...
from boat/webui/mcp/servers/ spawned over stdio - the same path the UI
exercises.
"""
...
from boat.webui.server import BoatWebUIServer  # noqa: E402


@pytest.fixture
def client():
    server = BoatWebUIServer(entities_dir=None)
...
def test_add_connect_call_disconnect_remove(client):
    ...
    assert (
        client.post("/api/mcp/servers/lab/disconnect").json()["status"]
        == "disconnected"
    )
...
def test_error_paths(client):
    ...
    response = client.post(
        "/api/mcp/servers", json={"server_id": "x", "transport": "stdio"}
    )
    assert response.status_code == 400
