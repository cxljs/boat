...
from boat.eval import AgentConfig, Dataset
...
from boat.optim.base import Candidate
from boat.types import EvalScore, RunTrajectory, Task, Usage
...
def _score(task, value, tokens=10):
    traj = RunTrajectory(
        ...
        success=True,
        usage=Usage(
            duration_ms=1, llm_calls=1, tokens_input=tokens, tokens_output=tokens
        ),
    )
...
def test_instruction_tunable_rewrite():
    ...
    new = spec.apply(
        cfg, [Edit(Operation.REWRITE, "instructions", "instructions", value="new")]
    )
...
def _cand(name, scores):
    ...
    c.scores = [
        _score(Task(name=f"t{i}", input="", id=f"t{i}"), s)
...
async def test_budget_caps_rollouts():
    ...
    opt = StubOptimizer(
        object(), ds, rounds=100, budget=20, score_fn=lambda c, t: len(c.system_prompt)
    )
    res = await opt.optimize(AgentConfig(name="seed", system_prompt="s"))
...
async def test_cost_log_populated_by_source():
    ds = Dataset(name="d", tasks=_tasks(2))
    opt = StubOptimizer(
        object(), ds, rounds=2, beam=20, score_fn=lambda c, t: len(c.system_prompt)
    )
    res = await opt.optimize(AgentConfig(name="seed", system_prompt="s"))
    ...
    assert total["tokens_total"] == (
        res.cost.agent.tokens_input
        + res.cost.agent.tokens_output
        + res.cost.judge.tokens_input
        + res.cost.judge.tokens_output
    )
...
def test_costlog_partial_usd_total_none():
    cl = CostLog()
    cl.agent = Usage(
        duration_ms=0, llm_calls=1, tokens_input=10, tokens_output=5
    )  # no usd
    cl.judge = Usage(
        duration_ms=0, llm_calls=1, tokens_input=10, tokens_output=5, cost_estimate=0.1
    )
    ...
    cl.agent = Usage(
        duration_ms=0, llm_calls=1, tokens_input=10, tokens_output=5, cost_estimate=0.05
    )
...
async def test_trace_records_gate_rejections_and_round_trips(tmp_path):
    ...
    from boat.optim import load_trace
    ...
    assert (
        len(res.pool) == 1
    )  # every proposal rejected -> result knows nothing about them
...
async def test_trace_records_accepted_candidates_and_noops():
    ...
    assert [e["purpose"] for e in tr.of_kind("eval")] == [
        "seed",
        "gate",
        "full",
        "gate",
    ...
    assert [(c["name"], c["parent"]) for c in cands] == [
        ("seed", None),
        ("c1", "seed"),
        ("c2", "c1"),
    ]
    assert (
        list(tr.of_kind("select"))[-1]["pool"][0] == "c2"
    )  # greedy keeps the best first
...
async def test_reflective_trace_keeps_prompt_and_structured_response():
    ...
    from boat.messages import AssistantMessage
    from boat.optim import ReflectiveOptimizer
    from boat.optim.reflective import _EditOut, _ReflectionOut
    from boat.types import ChatCompletionResult
    ...
    class FakeReflector:
        ...
        async def create(self, messages, output_format=None, **kw):
            ...
            out = _ReflectionOut(
                ...
                edits=[
                    _EditOut(
                        ...
                        kind="instructions",
                        name="instructions",
                        value="Always include TKT-######.",
                        rationale="judge asked for it",
                    )
                ],
            )
            return ChatCompletionResult(
                ...
                usage=Usage(
                    duration_ms=3, llm_calls=1, tokens_input=50, tokens_output=20
                ),
                model="fake-model",
                finish_reason="stop",
    ...
    class StubReflective(ReflectiveOptimizer):
        async def _eval(self, config, tasks):
            self.eval_count += len(tasks)
            return [
                _score(t, 10.0 if "TKT" in config.system_prompt else 2.0) for t in tasks
            ]

    ds = Dataset(name="d", tasks=_tasks(2))
    opt = StubReflective(object(), ds, rounds=1, reflector=FakeReflector())
    res = await opt.optimize(
        AgentConfig(name="seed", system_prompt="Help the customer.")
    )
    ...
    p = props[0]
    assert (
        p["stage"] == "reflect" and p["parent"] == "seed" and p["candidate"] == "seed#1"
    )
