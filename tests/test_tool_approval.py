...
from boat.messages import AssistantMessage, Message, ToolCallRequest, ToolMessage
from boat.tools import ApprovalMode, FunctionTool, tool
...
async def test_approval_flow_rejected():
    ...
    assert (
        ...
        or "rejected" in tool_messages[0].content.lower()
    )
...
async def test_approval_event_emission():
    ...
    async for item in agent.run_stream("Delete /tmp/test.txt"):
        if not isinstance(item, AgentResponse):
            # This is an event
            events.append(item)
...
@pytest.mark.asyncio
async def test_response_message_accessors():
    """Test AgentResponse.messages and final_content read from the context."""
    context = AgentContext()
