...
from boat.context import AgentContext
from boat.hooks import (
    BaseEndHook,
    BaseStartHook,
    CompletionCheckHook,
    LoopContext,
...
# Patch path for load_todos - it's imported lazily inside CompletionCheckHook
LOAD_TODOS_PATCH = "boat.tools.context_tools.load_todos"
...
class TestStartHook:
    ...
    async def test_planning_hook_injects_message(self):
        ...
        await agent.run("Build a website")

        assert client.call_count == 1
        first_call_messages = client.received_messages[0]

        hook_msgs = [
            m for m in first_call_messages if getattr(m, "source", "") == "hook"
        ]
...
class TestMaxRestarts:
    ...
    async def test_max_restarts_terminates(self):
        ...
        with patch(LOAD_TODOS_PATCH, return_value=always_incomplete):
            await agent.run("Do the impossible")
    ...
    async def test_restarts_dont_exceed_max_iterations(self):
        ...
        client = MockClient()
        client.set_responses(
            [AssistantMessage(content=f"Attempt {i}", source="mock") for i in range(20)]
        )
        agent = make_agent(
            client,
...
class TestTerminationConditions:
    ...
    def test_mixed_and_or(self):
        ...
        cond = (NeverTerminate() & MaxRestartsTermination(2)) | MaxRestartsTermination(
            5
        )
...
class TestTrajectory:
    ...
    async def test_start_and_end_hooks_together(self):
        ...
        with patch(LOAD_TODOS_PATCH, return_value=incomplete):
            await agent.run("Build something")
    ...
    async def test_tool_calls_with_end_hook(self):
        ...
        with patch(LOAD_TODOS_PATCH, return_value=incomplete):
            await agent.run("Use tool then stop")
...
class TestCompletionCheckHookUnit:
    ...
    async def test_returns_none_when_terminated(self):
        ...
        with patch(
            LOAD_TODOS_PATCH, return_value=[{"status": "pending", "content": "X"}]
        ):
