...
from boat.agents import Agent
from boat.context import AgentContext, ToolApprovalRequest
from boat.llm import BaseChatCompletionClient
from boat.messages import (
    AssistantMessage,
    ToolCallRequest,
    UserMessage,
)
from boat.middleware import (
    BaseMiddleware,
    GuardrailMiddleware,
    LoggingMiddleware,
    MetricsMiddleware,
...
from boat.tools import BaseTool
from boat.types import (
    AgentResponse,
...
class MockChatCompletionClient(BaseChatCompletionClient):
    ...
    async def create_stream(self, messages, tools=None, output_format=None, **kwargs):
        ...
        from boat.types import ChatCompletionChunk
...
class TestAgentMiddlewareIntegration:
    ...
    @pytest.mark.asyncio
    async def test_run_stream_pauses_on_middleware_approval_event(self, mock_agent):
        """run_stream pauses (no NameError) when middleware yields an approval event.

        Regression: the streaming model-call path referenced ToolApprovalEvent
        without importing it, so an approval-pausing middleware crashed the
        stream with NameError instead of pausing.
        """
        agent, client, context = mock_agent

        class ApprovalPausingMiddleware(BaseMiddleware):
            async def process_request(self, context: MiddlewareContext):
        ...
        agent.middleware_chain = MiddlewareChain([ApprovalPausingMiddleware()])
        client.set_response("Test response")
        ...
        # Paused before the model result was produced: no final AgentResponse
        assert not any(isinstance(e, AgentResponse) for e in events)
...
class TestBuiltInMiddleware:
    ...
    async def test_logging_middleware(self, caplog):
        ...
        with caplog.at_level(logging.INFO):
            async for item in logging_middleware.process_response(
                result_context, "test_result"
            ):
                pass
